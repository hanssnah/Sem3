...
CustomerOrder::CustomerOrder(const std::string& str) {
    ...
    if(m_product.length()> util.getFieldWidth()){
...
void CustomerOrder::display(std::ostream& os) const {
    ...
    if(m_lstItem && m_cntItem >0){
        for(size_t i =0; i<m_cntItem ; i++){
            os <<std::fixed<<"["<< std::right << std::setfill('0') << std::setw(6) << m_lstItem[i]->m_serialNumber <<  std::setfill(' ') << "] "
            <<std::setw(m_widthField)<< std::left << m_lstItem[i]->m_itemName << (m_lstItem[i]->m_fillState ? " - FILLED" : " - MISSING") <<std::endl;;
